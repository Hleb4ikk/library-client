    ...
    <header className="sticky top-0 z-50 border-b border-fern-dark/40 bg-fern/95 shadow-page backdrop-blur">
      <div className="mx-auto flex h-16 w-full max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        ...
        <nav className="flex items-center gap-2 sm:gap-4">
          <Button
            variant="ghost"
            className="flex bg-natural/15 px-4 py-2 text-apricot hover:bg-natural/25 items-center gap-2"
            onClick={() => navigate("/")}
          >
            <Search size={18} />
            <span className="hidden sm:flex">Найти</span>
          </Button>
          ...
          {isLoading ? (
            ...
            <>
              <div ref={cabinetRef} className="relative">
                <Button
                  variant="ghost"
                  className="flex items-center px-3 py-2 text-ivory hover:bg-natural/15 md:hidden whitespace-nowrap"
                  onClick={() => setIsCabinetOpen((current) => !current)}
                ...
                {isCabinetOpen && (
                  <div className="absolute right-0 top-12 w-52 overflow-hidden rounded-2xl border border-natural/25 bg-ivory-card py-2 shadow-dropdown">
                    {cabinetLinks.map((link) => (
                      ...
                      >
                        <span className="w-4 text-apricot">{link.icon}</span>
                        {link.label}
                      </button>
                    ))}
                  </div>
