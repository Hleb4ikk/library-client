...
const statusFilters: Array<{ label: string; value: StatusFilter }> = [
  { label: "Все", value: "all" },
  { label: "Хочу прочитать", value: EStatusBadgeVariant.WANT },
  { label: "Читаю сейчас", value: EStatusBadgeVariant.READING },
  { label: "Прочитано", value: EStatusBadgeVariant.DONE },
];

const statusOptions = Object.values(EStatusBadgeVariant);
...
function ReadingListBookItem({
  ...
  const badgeOptions: Record<EStatusBadgeVariant, ButtonVariants> = {
    [EStatusBadgeVariant.DONE]: "done",
    [EStatusBadgeVariant.READING]: "reading",
    [EStatusBadgeVariant.WANT]: "want",
  };
    ...
    <article className="rounded-2xl border border-natural/25 bg-ivory px-4 py-4 shadow-card transition hover:shadow-page">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
        ...
        >
          ...
          <p className="mt-1 text-sm text-natural-text">{book.author}</p>

          <div className="mt-3">
            <Badge variant={badgeOptions[status]}>{status}</Badge>
          </div>
        </button>
        ...
        <div className="flex items-center gap-2 sm:min-w-[240px] sm:justify-end">
          <select
            value={status}
            onChange={(event) =>
              onStatusChange(book.id, event.target.value as EStatusBadgeVariant)
            }
            ...
            <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
              <div>
                ...
                <p className="mt-2 text-sm text-natural-text">
                  Управляйте книгами, меняйте статус и открывайте детали прямо
                  из списка.
                </p>
              </div>
            ...
            <div className="mt-6 flex flex-wrap gap-2">
              {statusFilters.map((filter) => (
                <Button
                  key={filter.value}
                  variant={
                    activeFilter === filter.value ? "secondary" : "ghost"
                  }
